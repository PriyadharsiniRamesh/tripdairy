	...
	<body>
		...
		<div class="sub-plan">
				...
				<?php
				  if( mysql_num_rows( $selectRes )==0 ){
					...
					while( $row = mysql_fetch_assoc( $selectRes ) ){
							echo "<tr><td>{$row['name']}</td><td>{$row['mail']}</td><td>{$plan}</td><td>{$stime}</td><td>{$etime}</td></tr>\n";
					}
				  }
				?>
			  </tbody>
			...
			<form method="POST" action="">
				<div id="form-row"><input type="submit" value="Cancel" name="cancel" class="btns"/>
				<input type="submit" value="Confirm" name="confirm" class="btns" style="margin-left:100px;"/>
			</form>
			<?php 
				if(isset($_POST['confirm'])){
					$con=mysql_connect('localhost','root','') or die(mysql_error());  
					mysql_select_db('tripdairy_user') or die("cannot select DB");  
					$sql="UPDATE user SET plan='$plan',start_time='$stime',end_time='$etime' WHERE mail='$user'";  
					$result=mysql_query($sql);  
						...
						else {echo "<script type='text/javascript'>alert('Some error has occured..!');</script>";}
				}
